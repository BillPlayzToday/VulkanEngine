...
private:
	...
	void createInstance() {
		if (enableValidationLayers && !checkValidationLayerSupport()) {
			throw std::runtime_error("validation layers not available on request!");
		}
	...
	void createImageViews() {
		...
		for (size_t i = 0; i < swapChainImages.size(); i++) {
			...
			if (vkCreateImageView(device, &createInfo, nullptr, &swapChainImageViews[i]) != VK_SUCCESS) {
				throw std::runtime_error("can't create one image view!");
			}
		}
	}
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE); // START
		...
		vkCmdEndRenderPass(commandBuffer); // END
	...
	void createSyncObjects() {
		...
		for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
			if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
				...
				throw std::runtime_error("can't create synchronization objects for a frame!");
			}
		}
	}
	...
	void drawFrame() {
		...
		if (vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) {
			throw std::runtime_error("can't submit draw command buffer!");
		}
	...
	void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
		...
		vkBeginCommandBuffer(commandBuffer, &beginInfo); // START
		VkBufferCopy copyRegion{};
		copyRegion.size = size;
		vkCmdCopyBuffer(commandBuffer, srcBuffer, dstBuffer, 1, &copyRegion);
		vkEndCommandBuffer(commandBuffer); // END
